...
from models.users import Users  # Ensure this import matches your project structure
from flask_login import current_user, login_user, UserMixin, logout_user
...
class TestFlaskRoutes(unittest.TestCase):
    ...
    def test_home_route_get(self):
        ...
        self.assertEqual(response.status_code, 302)  # Assuming home route returns HTTP status code 200

    def test_home_route_post(self):
        """ Test the POST request to the home route """
        response = self.app.post('/home', json={'tableData1': [], 'tableData': [], 'tableData2': []})
        self.assertEqual(response.status_code, 302)  # Assuming home route returns HTTP status code 200

    def test_about_us_route(self):
        """ Test the GET request to the about us route """
        response = self.app.get('/about_us')
        self.assertEqual(response.status_code, 200)  # Assuming about us route returns HTTP status code 200

    def test_contact_us_route(self):
        """ Test the GET request to the contact us route """
        response = self.app.get('/contact_us')
        self.assertEqual(response.status_code, 200)  # Assuming contact us route returns HTTP status code 200
    ...
    def test_index_route_post(self):
        ...
        self.assertTrue(check_password_hash(new_user.password, test_password))
...
class TestLoginRoute(unittest.TestCase):
    ...
    def test_login_successful_redirect(self, mock_storage):
        ...
        mock_user = Users(username='test_user', email='test_user@example.com', password='hashed_password')
        mock_storage.get_by_username.return_value = mock_user

        # Mock password hash check
        with patch('web_dynamic.OptiDash.check_password_hash', return_value=True):
    ...
    def test_login_invalid_credentials(self, mock_storage):
        ...
        # Mock password hash check
        with patch('web_dynamic.OptiDash.check_password_hash', return_value=False):
            response = self.app.post('/login', data=form_data)

        # Check that the user is redirected to the login page with an error message
        self.assertEqual(response.status_code, 200)
...
class TestLogoutRoute(unittest.TestCase):
    ...
    @patch('web_dynamic.OptiDash.logout_user', side_effect=logout_user)
    def test_logout(self, mock_logout_user):
        with app.app_context():
            with self.app as c:
                with c.session_transaction() as sess:
                    # Mock login_user to simulate a logged-in user
                    test_user = MockUser(3)  # Use an integer ID instead of 'test_user_id'
