...
def get_all_optimizationresult():
    """ Retrieves the list of all
        Optimization Results objects.
    """
    optimizationresults = [
        optimizationresult.to_dict()
        for optimizationresult in
        storage.all(OptimizationResult).values()]
    return jsonify(optimizationresults)


@app_views.route(
        '/optimizationresults/<optimizationresult_id>',
        methods=['GET'], strict_slashes=False)
def get_optimizationresult_by_id(optimizationresult_id):
    """Retrieves a optimization result object."""
    optimizationresult = storage.get(
        OptimizationResult,
        optimizationresult_id)
...
@app_views.route(
        '/problems/<int:problem_id>/optimization_results',
        methods=['GET'],
...
def get_optimization_results_by_problem_id(problem_id):
    ...
    """
    problem_results = storage.get_optimization_results_by_problem_id(
        problem_id)
    if not problem_results:
        abort(404, description="Problem not found or no optimization"
